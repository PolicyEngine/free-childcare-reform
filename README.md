...
**Which take-up flag.** The subsidy is defined for `would_claim_tfc`, Tax-Free Childcare's take-up input. The alternative put to us — read take-up from the extended (30-hour) entitlement's flag instead, since the reform drops TFC's restrictions — is costed alongside it in `subsidy_take_up.extended_entitlement_flag`. It moves the leg the other way from the argument for it: on this data the TFC flag is the higher of the two (0.87 against 0.80 among families with a qualifying child, uncorrelated with each other and with anything else), so the subsidy leg falls from £4.84bn to £4.31bn and its net entrants from about 11,900 to 9,200 in 2027-28. Neither flag is estimated for this reform. The switch is one line, `reforms.SUBSIDY_TAKE_UP_FLAG`.
