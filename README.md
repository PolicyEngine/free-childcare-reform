...
**The labour supply response is small and, on the central assumption, negative.** Free hours give **−17,014** net entrants — making 15 hours unconditional removes a reason to work — while the subsidy gives **+7,078**. Together, **−10,361**. The fiscal effect is under £0.02bn.

Two corrections moved this from the **+5,558** an earlier version reported. A double-count: baseline Tax-Free Childcare was subtracted from out-of-work income even though the out-of-work recomputation had already removed it, since it is work-tested. And the earnings-quintile classification, which indexes the OBR elasticities, ranked adults by cumulative weight rather than placing them against the earnings distribution's thresholds — ranking splits a tied mass by input row order, so identical people drew different elasticities by file position. Neither the sign nor the size should be leaned on: placing imputed entrant earnings at full-time equivalent, as policyengine-uk does, would give about **−14,600** instead.
